...
import json, pickle    #, cPickle    There is no cPickle in Python 3. Just import pickle. pickle will automatically use the C accelerator.
...
def timing(f):
    ...
    def wrap(*args, **kw):
        ...
        delta = te-ts
        TOTAL_TIMING += delta
        print(f" func {f.__name__}   args {args}  {kw}  took  {delta}  sec   ;  TOTAL_TIMING {TOTAL_TIMING}")
...
deck = [x for x in range(1,DECK_MAX_RANGE)]
...
while EXPLORATION:
    ...
    #ai.save_json()    TypeError:  keys must be str, int, float, bool or None, not bytes
...
while AGAIN:
    ...
    for player in Players:
        ...
        if player.human is False and player.alive is True:
            ...
            for code_action in ai.states_actions[state_now]:
                some_reward = judge_dredd.genial(code_action, Players)
                print(f"code_action  {code_action}    some_reward = {some_reward} ")


                ai.states_actions_rewards[state_now][code_action] = some_reward

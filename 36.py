	...
	def random_action(self):
		...
		if not self.hand == []:
			...
			if len(self.hand) >= 2:
				...
				if act == 0:
					self.fill_palette()
					print(f"---------0-------- player {self.idx_player}  fill_palette   ->  {self.pal}")

				elif act == 1:
					self.fill_canvas()
					print(f"--------1--------- player {self.idx_player}  fill_canvas")
				else:
					self.fill_palette()
					self.fill_canvas()
					print(f"--------2--------- player {self.idx_player}  fill_palette _AND_ fill_canvas")
